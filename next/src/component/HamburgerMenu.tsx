      ...
      >
        <span
          className={`block h-1 w-6 rounded-sm bg-red-600 
                    transition-all duration-300 ease-out ${isOpen ? "translate-y-2 rotate-45" : "-translate-y-0.5"
            }`}
        ></span>
        <span
          className={`my-0.5 block h-1 w-6 rounded-sm 
                    bg-red-600 transition-all duration-300 ease-out ${isOpen ? "opacity-0" : "opacity-100"
            }`}
        ></span>
        <span
          className={`block h-1 w-6 rounded-sm bg-red-600 
      ...
      {isOpen && (
        <div className="fixed left-0 top-0 flex h-screen w-screen items-center flex-col justify-center bg-zinc-900 z-30">
          <div className="flex space-x-2 justify-center items-center">
            ...
            <p className="text-2xl font-semibold mt-auto text-white">Active Living</p>
          </div>
